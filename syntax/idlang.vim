...
"no <f8> :!idlpro `basename %:t .pro`<cr>
"no <s-f8> :!idlpror `basename %:t .pro`<cr>

syn case ignore
"set fdm=syntax
"syn cluster idlangCaseEndcaseList contains=idlangCaseStart,idlangCase,idlangCaseBar,idlangCaseIn,idlangComment,idlangDeref,idlangDerefSimple,idlangCaseCommandSub,idlangCaseExSingleQuote,idlangCaseSingleQuote,idlangCaseDoubleQuote,idlangSpecial
...
syn keyword idlangConditional if else then for while case switch repeat until
syn keyword idlangConditional endcase endelse endfor endswitch endif endrep endwhile

syn match idlangOperator " and \| or \| xor \| eq \| ge \| gt \| le \| lt \| ne "me=e-1
syn match idlangOperator " && \| || \| > \| < "me=e-1
syn match idlangOperator /\(\ \|(\)not\ /hs=e-3
syn match idlangOperator " \~[^\/]"me=e-1 skipwhite
...
syn match idlangNumber "-\|+\|\*\|\/ " nextgroup=idlangRealNumber,idlangNumber
syn match idlangNumber "-=\|+=\|*=\|\/="me=e-1 skipwhite

syn match  idlangComment "[\;].*$" contains=idlangTodo

syn match idlangContinueLine "\$\s*\($\|;\)"he=s+1 contains=idlangComment
syn match idlangContinueLine "&\s*\(\p\|;\)"he=s+1 contains=ALL
...
syn match idlangRoutine "HDF_SD_\a*"
syn match idlangRoutine "HDF_VD_\a*"
...
syn keyword idlangRoutine H5F_OPEN H5D_OPEN H5D_READ H5D_CLOSE H5F_CLOSE
...
syn keyword idlangRoutine BIT_FFS BIT_POPULATION BUTTERWORTH CANNY
...
syn keyword idlangRoutine PATH_PREFS PATH_SEP PICKFILE PRODUCT
...
so ~/.vim/syntax/my_idl_routines.vim
syn match idlangStrucvar "^@.*$"
syn keyword idlangStop stop pause

"
"DO NOT PLAY with the syn-fold (commented below)!!!
"
"
"if g:sh_fold_enabled
"sy region idlangCaseEndcase fold matchgroup=idlangConditional start="\<case\>" end="\<endcase\>" contains=@idlangCaseEndcaseList
"sy region idlangCaseEndcase start="\<case\>" end="\<endcase\>" contains=ALL fold keepend
"exec "syn sync minlines=4 maxlines=400"
"sy sync match idlangCaseEndcaseSync grouphere idlangCaseEndcase "\<case\>"
"sy sync match idlangCaseEndcaseSync groupthere idlangCaseEndcase "\<endcase\>"
...
if version >= 508 || !exists("did_idlang_syn_inits")
  ...
  HiLink idlangStop	SpellBad
  HiLink idlangStrucvar	PreProc
  HiLink idlangSystem	Identifier
  HiLink idlangKeyword	Special
  HiLink idlangBrackets	PreProc
"type :hi<cr> to see all possible colorings
